...
const Api = {
  ...
  // Summarize
  summarizeText:    (text, options) => Api.post('/summarize/text', { text, options, apiKey: Auth.getApiKey() }),
  summarizeURL:     (url, options)  => Api.post('/summarize/url',  { url,  options, apiKey: Auth.getApiKey() }),
  summarizeYoutube: (url, options)  => Api.post('/summarize/youtube', { url, options, apiKey: Auth.getApiKey() }),
    ...
    <div class="result-box fade-in">
      ...
      ${result.sourceInfo ? `
        <div style="margin-top:24px;padding:16px;background:rgba(255,255,255,0.03);border-radius:var(--radius-md);font-size:0.82rem;color:var(--text-muted)">
          <strong>Source Info:</strong> ${result.sourceInfo.pages ? `📄 ${result.sourceInfo.pages} pages` : ''} 
      ...
      <div style="margin-top:20px;padding-top:16px;border-top:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;font-size:0.75rem;color:var(--text-muted)">
        <span>Generated by ${result.model || 'Gemini 1.5 Flash'}</span>
...
function downloadSummary() {
  ...
  const content = [
    ...
    `Model: ${r.model || 'Gemini 1.5 Flash'}`,
    '', '## Summary', r.summary || '',
    '', '## Key Points',
    ...(r.keyPoints || []).map(p => `- ${p}`),
    '', '## Tags',
